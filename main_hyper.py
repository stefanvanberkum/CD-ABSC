...
def run_hyper(domain, year, size, fine_tune, runs, n_iter):
    path = "hyper_results/fine_tuning/" + domain + "/" + str(n_iter) + "/"
    if fine_tune:
        path = "hyper_results/fine_tuning/" + domain + "/" + str(n_iter) + "/"
        train_path = "data/programGeneratedData/BERT/" + domain + "/768_" + domain + "_train_" + str(
            year) + "_BERT_" + str(size) + ".txt"
    else:
        path = "hyper_results/LCRROT_ALTV4/" + domain + "/" + str(n_iter) + "/"
        train_path = "data/programGeneratedData/BERT/" + domain + "/768_" + domain + "_train_" + str(
            year) + "_BERT_" + str(size) + ".txt"
    ...
    def lcr_altv4_objective(hyperparams):
        ...
        l, pred1, fw1, bw1, tl1, tr1 = lcr_model.main(FLAGS.hyper_train_path, FLAGS.hyper_eval_path,
                                                      accuracyOnt,
    ...
    def lcr_fine_tune_objective(hyperparams):
        ...
        l, pred1, fw1, bw1, tl1, tr1 = lcr_fine_tune.main(FLAGS.hyper_train_path,
